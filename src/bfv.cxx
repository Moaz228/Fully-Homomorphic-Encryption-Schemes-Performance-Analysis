...
int main(int argc, char *argv[]) {
  ...
  if (!decryptMode) {
    ...
    if (fileExists("cryptocontext.bin") && fileExists("public_key.bin")) {
      std::cout << "[BFV] Loading existing keys..." << std::endl;
      ...
      std::cout << "[BFV] Generating new context..." << std::endl;
    ...
    while (inputFile >> temp_val)
      gateway_input.push_back((int64_t)(temp_val * 100)); // Scale to int
    ...
    std::cout << "[BFV] Encryption Time: " << time_duration_ms(enc) << " ms"
              << std::endl;
    ...
    std::cout << "[BFV] Decryption Time: " << time_duration_ms(dec) << " ms"
              << std::endl;
